...
public interface OrdersLotteryActivityRep extends BaseRepository<OrdersLotteryActivity> {
    ...
    @Query("select o from OrdersLotteryActivity o where ?1>= o.startTime and ?2 > o.startTime and o.status = 1 and  o.id is not null and id<>?2")
    List<OrdersLotteryActivity> findByDateId2(Date date1,Date date2, Long id);

    @Query("select o from OrdersLotteryActivity o where ?1>= o.endTime and ?2 > o.endTime and o.status = 1 and  o.id is not null and id<>?2")
